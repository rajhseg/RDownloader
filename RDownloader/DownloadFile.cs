...
namespace RDownloader
{
	...
	public partial class DownloadFile : Form
	{
		...
		private void UpdateProgressChanged(string id, double size, UpdateTypeSize typeSize)
		{
			...
			} else if(typeSize == UpdateTypeSize.UpdatePartResponseChunked){
				this.UpdatePartResponseChunked(id);
			}
		}
		
		private void UpdatePartResponseChunked(string id){
			if (!string.IsNullOrEmpty(id)) {
				var _id = parts.FirstOrDefault(x => x.PartId == id);
				if (_id != null) {
					_id.IsResponseChunked(true);
				}
			}			
		}
		...
		public DownloadFile(string url)
		{
			...
			this.overallTimer.Tick += (s, e) => {
				overallStatus.Text = this.GetOverallStatus();
			};
		...
		public void Download(object sender, DoWorkEventArgs e)
		{
			...
			if (isChunked) {
				...
				this.ReportProgressState(part.PartId, 0, UpdateTypeSize.UpdatePartResponseChunked);
													
				this.ReportProgressState(part.PartId, 0, UpdateTypeSize.TotalSize);
				
				var task_part = new Task(() => StartDownloadFromChunked(URL, finalPath, part, token.Token, e), token.Token);	
				task_part.Start();
	            
				var param = this.CreateTaskParameters(task_part.Id, task_part, URL, finalPath, part, e);
				allTasks.Add(param);	            
				...
				for (long i = (long)partSize; i <= responseLength; i = i + (long)partSize) {	        		        	
					...
					this.ReportProgressState(part.PartId, partSize, UpdateTypeSize.TotalSize);
					
					var task_part = new Task(() => StartDownload(URL, finalPath, startPosition, endPosition, part, token.Token, e), token.Token);	
									
					var param = this.CreateTaskParameters(task_part.Id, task_part, URL, finalPath, part,e,startPosition, endPosition);
					allTasks.Add(param);	            
				...
				if (remainder > 0) {		        	
					...
					this.ReportProgressState(part.PartId, remainder, UpdateTypeSize.TotalSize);
												
					var task_part = new Task(() => StartDownload(URL, finalPath, startPosition, endPosition, part, token.Token, e), token.Token);						
		            					
					var param = CreateTaskParameters(task_part.Id, task_part, URL, finalPath, part,e,startPosition, endPosition);
		...
		public void ReRunPartSubMethod(TaskParameters task, DownloadParts part)
		{
			...
			this.ReportProgressState(part.PartId, 0, UpdateTypeSize.DownloadSize);
			
			this.RunTaskParameters(task);	
				
			var pstatus = new UpdatePartStatus() {
				Part = part,
				Message = "Queued, ReStarting, Please Wait..."
			};

			this.ReportProgressState(part.PartId, 0, UpdateTypeSize.UpdatePartStatus, pstatus);			
		}
		...
		private void RunParallel(List<TaskParameters> tasksParams, DoWorkEventArgs e)
		{
			...
			if (isAllFinished && !this.mergingStarted && allAreHundredPercent) {
		        ...
		        this.ReportProgressState("0", 0, UpdateTypeSize.UpdateOverallStatus, "Merging....");		        
				this.mergeClean();
				this.completed = true;
				return;
			}			
			...
			try {
				...
				using (Stream respStream = stream) {
					...
					while ((bytesRead = respStream.Read(buffer, 0, 4096)) > 0) {
						...
						this.ReportProgressState(part.PartId, size, UpdateTypeSize.DownloadSize);										
					}
		...
		{        	
			...
			using (var output = File.Create(Path.Combine(docPath, downloadFileName))) {
				...
				foreach (var file in Files) {
					...
					this.ReportProgressState("0", 0, UpdateTypeSize.UpdateOverallStatus, "Merging....  " + percentage + "%");
		...
		private void ReportProgressState(string id, double size, UpdateTypeSize typeSize, object obj = null)
		{
			ProgressState sta2 = new ProgressState();
			sta2.ID = id;
			sta2.Size = size;
			sta2.TypeSize = typeSize;
			sta2.Object = obj;
					
			this.worker.ReportProgress(0, sta2);												
		}
		
		private TaskParameters CreateTaskParameters(int taskid, Task task_part, string url, string finalPath, 
		                                            DownloadParts part, DoWorkEventArgs e, long startIndex = default(long), long endIndex = default(long))
		{
		...
		void DownloadFileLoad(object sender, EventArgs e)
		{
			...
			this.worker.RunWorkerAsync(new object[] {
				_downloadFileUrl,
				this.TotalResponseLength,
				this.IsReponseChunked
			});				
